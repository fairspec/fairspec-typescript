...
describe("parseDateColumn", () => {
  ...
  it.each([
    ["2006/11/21", new Date(Date.UTC(2006, 10, 21))],
  ])("temporalFormat %%Y/%%m/%%d: %s -> %s", async (cell, expected) => {
    const table = pl.DataFrame([pl.Series("name", [cell], pl.String)]).lazy()
    const tableSchema: TableSchema = {
      properties: {
        name: {
...
describe("stringifyDateColumn", () => {
  ...
  it.each([
    [new Date(Date.UTC(2006, 10, 21)), "21/11/2006"],
  ])("temporalFormat %%d/%%m/%%Y: %s -> %s", async (value, expected) => {
    const table = pl.DataFrame([pl.Series("name", [value], pl.Date)]).lazy()
    const tableSchema: TableSchema = {
      properties: {
        name: {
  ...
  it.each([
    [new Date(Date.UTC(2006, 10, 21)), "2006/11/21"],
  ])("temporalFormat %%Y/%%m/%%d: %s -> %s", async (value, expected) => {
    const table = pl.DataFrame([pl.Series("name", [value], pl.Date)]).lazy()
    const tableSchema: TableSchema = {
      properties: {
        name: {
